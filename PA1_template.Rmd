...
Histogram of the total number of steps taken per day (treating NA as 0).  The NAs have a big effect on this graph because a day that is entirely NAs will seem to have 0 steps.
```{r echo=TRUE}
...
The mean number of steps per day is `r mean(daily.totals, na.rm=T)`; the median is `r median(daily.totals, na.rm=T)`.
...
```
Demonstrating embedding this value in a sentence: The total number of missing values is `r sum(is.na(activity$steps))`.

To impute missing values, I will use the mean for that 5-minute interval across all the days for which that interval is not NA.  The **imputed** data frame will be equal to **activity**, but with the missing values filled in.
```{r echo=TRUE}
imputed <- activity
...
The mean and median steps per day, using the imputed data.  **These are higher than they had been in the non-imputed data set.**
```{r echo=TRUE}
...
The patterns on weekdays and weekends look different.  Weekdays have a high peak early in the day, during which the person is probably exercising or hurrying to work.  This is followed by a fairly sedentary day.  Weekends also have an early peak, which is lower than that of the weekdays, and the weekend activity during the rest of the day is higher than on weekdays.  The person appears to go to bed later on the weekends than in the weekdays (on average).
```{r echo=TRUE}
